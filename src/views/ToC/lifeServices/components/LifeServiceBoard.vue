<template>
  <main class="life-service-board">
    ...
    <section v-loading="loading" class="life-service-surface">
      ...
      <div v-if="pagedRecords.length" class="life-service-grid">
        <article v-for="item in pagedRecords" :key="item.id" class="life-service-card" tabindex="0" role="button" @click="openDetail(item)" @keydown.enter="openDetail(item)">
          <div class="card-head">
    ...
    <el-dialog
      ...
      custom-class="life-service-form-dialog"
      @closed="resetForm">
      <el-form ref="lifeServiceForm" :model="form" :rules="rules" label-width="92px" @submit.native.prevent>
        <el-row :gutter="28">
    ...
    <el-drawer :title="config.title + '详情'" :visible.sync="detailVisible" size="500px" append-to-body :lock-scroll="true" custom-class="life-service-detail-drawer">
...
export default {
  ...
  methods: {
    ...
    // 项目 Axios 拦截器通常已返回业务 data；仅保留兼容解包逻辑。
    unwrap(response) {
    ...
    statusText(value) {
      if (value === undefined || value === null || value === '' || value === '0') return this.config.defaultStatus || '正常'
    ...
    statusType(status) {
      ...
      if (['待审核'].includes(status)) return 'warning'
...
@media (max-width: 760px) {
  ...
  ::v-deep .life-service-detail-drawer {
    width: 100% !important;
    max-width: 100% !important;
  }
  ::v-deep .life-service-detail-drawer .el-drawer__body {
    display: flex;
    min-height: 0;
    flex: 1 1 auto;
    overflow: hidden;
  ...
  ::v-deep .life-service-form-dialog .el-dialog__body {
    min-height: 0;
    flex: 1 1 auto;
    overflow-y: auto;
    padding: 18px 16px;
  ...
  ::v-deep .life-service-form-dialog .el-dialog__footer {
    flex: 0 0 auto;
