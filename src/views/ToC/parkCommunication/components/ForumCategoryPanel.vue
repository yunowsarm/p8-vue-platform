<template>
  <aside class="board-panel" aria-label="论坛版块">
    ...
    <button v-for="category in categories" :key="category.id" type="button" class="board-item" :class="{ active: activeCategory === category.id }" @click="$emit('select-category', category.id)">
