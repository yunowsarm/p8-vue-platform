<!-- 园区话题帖子列表：展示帖子内容、互动操作及“我的帖子”审核状态。 -->
<template>
      ...
      <article v-for="topic in topics" :key="topic.id" class="topic-card" tabindex="0" role="button" @click="$emit('open-topic', topic)" @keydown.enter="$emit('open-topic', topic)">
        <div class="topic-card-main">
          <div class="topic-topline">
            ...
            <el-tag v-if="showReviewStatus" size="mini" :type="reviewStatusType(topic.status)">{{ reviewStatusText(topic.status) }}</el-tag>
        ...
        <div class="topic-actions" @click.stop>
          <button v-if="!isReviewRestricted(topic)" type="button" :class="{ active: topic.liked }" :aria-label="'点赞 ' + topic.title" @click="$emit('toggle-like', topic)">
          ...
          <button v-if="!isReviewRestricted(topic)" type="button" aria-label="查看评论" @click="$emit('open-topic', topic)">
            <i class="el-icon-chat-dot-round"></i>
            {{ topic.replyCount }}
          </button>
          <button v-if="!isReviewRestricted(topic)" type="button" :class="{ active: topic.favorited }" :aria-label="'收藏 ' + topic.title" @click="$emit('toggle-favorite', topic)">
          ...
          <button v-if="isOwnTopic(topic)" type="button" class="topic-manage-button topic-manage-start danger" :aria-label="'删除 ' + topic.title" @click="$emit('delete-topic', topic)">
...
export default {
  ...
  props: {
    ...
    isReviewRestricted: { type: Function, required: true },
    showReviewStatus: { type: Boolean, default: false }
  },
  methods: {
