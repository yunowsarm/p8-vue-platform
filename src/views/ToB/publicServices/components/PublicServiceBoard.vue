  ...
  computed: {
    boardConfig() {
      ...
      return Object.assign({}, this.config, {
        ...
        defaultStatus: this.config.defaultStatus || (isConsultation ? '待受理' : '已发布'),
        statusOptions: this.config.statusOptions || (isConsultation ? ['待受理', '处理中', '已回复', '已关闭'] : ['草稿', '待审核', '已发布', '已下线']),
        summaryDefinitions: this.config.summaryDefinitions || (isConsultation
          ? [
