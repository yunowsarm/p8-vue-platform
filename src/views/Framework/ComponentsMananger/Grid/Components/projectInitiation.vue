        ...
        >
          <template :slot="item" v-for="item in customColumn" slot-scope="{ scope }">
            <template v-if="item === 'NAME'">
              <div :key="item">
                <span v-if="scope.row.STATUS === '2205'" class="base-custom-style-task approve" @click.stop="withdrawTaskApprove(scope.row)">审</span><span>{{ scope.row.NAME }}</span>
              </div>
            </template>
          </template>
        </common-table>
...
export default {
  ...
  methods: {
    ...
    withdrawTaskApprove(rowInfo) {
      const taskId = rowInfo.ID
      const url = 'baseData.withdraw'
      const _this = this
      _this
        .$confirm('是否要撤回审批', '提示', {
        ...
        .then(() => {
          _this.$api[url]({ withdrawList: [{ businessKey: taskId, processDefinitionKey: 'projectReadyApproveRelease' }] }).then((res) => {
            if (res.result) {
        ...
        if (res.reportItems && res.reportItems.length) {
          ...
          res.reportConfig.forEach((item) => {
            ...
            if (item.isCustomColumn == '1' && item.customColumnType === 'slot' && item.columnConfig.slotName) {
              ...
              console.log(this.customColumn, 'customColumncustomColumn')
            }
