<template>
  <normal-layout :split-layout="true" :header-visible="false">
    <template #north> </template>
    <template #west>
      <common-tree ref="commonTree" :node-slot="true" :data="treeData" :allow-drop="handlerAllowDrog" :allow-drag="handlerAllowDrag" :draggable="treeSettingsParmars.dataType === '1'">
        <template #tree="{ node }">
          <div style="width: 100%">
            <div class="labelShow">{{ node.label }}</div>
            <div class="iconOperation">
              <!-- dataType数据类型 0为动态数据  1为静态数据 -->
              <i class="el-icon-circle-plus" v-if="treeSettingsParmars.dataType === '1'" @click.stop="create(node.data)"></i>
              <i class="el-icon-edit" v-if="treeSettingsParmars.dataType === '1' || node.data.id === '0'" @click.stop="edit(node.data)"></i>
              <i class="el-icon-sort" v-if="treeSettingsParmars.dataType === '1' || node.data.id === '0'" @click.stop="paramsMapClick(node.data)"></i>
              <i class="el-icon-delete-solid" v-if="node.data.id !== '0' && treeSettingsParmars.dataType === '1'" @click.stop="deleteNode(node.data)"></i>
            </div>
          </div>
        </template>
      </common-tree>
    ...
    <template #center>
      <div v-if="!asyncComponents" class="text">系统按钮区自动载入菜单权限设置</div>
      <el-button class="delectButton" v-if="asyncComponents" @click="settingsDelect"><i class="el-icon-delete" style="margin-right: 5px"></i>修改</el-button>
    ...
    <template #drawer-panel>
      ...
      <child-node-settings
        ...
        @handleOk="ChildNodeSettingsHandleOk"
      ></child-node-settings>
      <params-map v-if="visibleParamsDrawer" :visible="visibleParamsDrawer" :params-list="paramsList" @close="visibleParamsDrawer = false" @handleOk="ParamshandleOk"></params-map>
    </template>
  </normal-layout>
</template>
