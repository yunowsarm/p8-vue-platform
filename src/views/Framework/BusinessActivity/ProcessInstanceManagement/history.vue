  ...
  <div class="container">
    <div class="approveContent">
      <vue-perfect-scroll class="scroll-area">
        ...
        <div>
          <form-list v-if="formValidate" ref="form" label-width="120px" :data-source="dataSource" :exist-default-btn="false" :api="saveApi" :form="formData">
            <template #uploadFiles>
              <div class="edit-outputdata-view">
                <ul class="file-list">
                  <li v-for="item in formData.uploadFiles" :key="item.id">
                    <p>
                      <span class="filename" @click="downloadOutputRequsetFile(item)">
                        {{ item.fileName }}
                      </span>
                    </p>
                  </li>
        ...
        <common-tabs class="custom-tabs" type="border-card" :active-tabs="activeTabs" :tabs-data="tabs">
          <template #approval>
            <component :selected-approval="selectedApproval" :curr-entity-id="currEntityId" v-if="formComp != null && formComp != ''" :is="componentLoader" v-bind="formCompProp" />
          </template>
          <template #bpmn>
            <bpm-view
          ...
          <template #history>
            <ProcessHistoryList v-if="processInstId" :table-api="historyDataApi" :columns="historyColumns" :process-inst-id="processInstId" :business-key="businessKey" :table-flex="tableFlex">
            </ProcessHistoryList>
          </template>
        </common-tabs>
      </vue-perfect-scroll>
