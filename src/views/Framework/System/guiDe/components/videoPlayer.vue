<template>
    <div class="demo1-video" id="video"  style="height:100%;width:100%;">
      <video
        id="myVideo"
...
export default {
  ...
  async mounted() {
    if (this.record.vTitle.length) {
      // 网络请求
      let res =  await this.$api['SystemSettings.getFileUrl']({ attachmentId: this.record.vTitle[0].id }, { responseType: 'blob' })
      if (res) {
        const blob = new Blob([res.data], { type: 'video/mp4' });
        const url = URL.createObjectURL(blob);
        this.videoUrl = url
      }
    } else if (this.record.vURL) {
      // 视频地址
      this.videoUrl = this.record.vURL
    }
    //创建一个视频对象
    const that = this;
     this.width = document.getElementById("video").offsetWidth
    this.height = document.getElementById("video").offsetHeight
    this.video = $video("myVideo", {
      language: "zh-CN",
