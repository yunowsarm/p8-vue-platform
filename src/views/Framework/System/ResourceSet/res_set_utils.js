...
const validResTypes = function (resObject) {
  let dictArray = []
  const config = _.find(resTypesConfigs, (o) => _.isMatch(resObject, o.rule))
...
const getColumns = function (resType, comp) {
  // 英文名称的唯一性校验需要用到vue组件中this对象，在getColumns时传入备用。
  thisComponent = comp
  const typeColFields = _.get(editFormConfigs, resType, [])
