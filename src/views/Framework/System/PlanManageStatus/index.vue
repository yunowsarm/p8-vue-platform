...
export default {
  ...
  methods: {
    ...
    getStatusList(kTeamId) {
      let that = this
      let statusKey = []
      that.$api[that.tableApi]({ dicType: 'PLAN_MANAGE_STATUS' }).then(function (res) {
        let dataTemp = []
        res.forEach(function (item) {
          let temp = {}
          temp.id = item.value
          temp.name = item.label
          temp.iconKey = 'planManagerStatus-icon-' + item.value
          temp.colorKey = 'planManagerStatus-color-' + item.value
          dataTemp.push(temp)
          statusKey.push('planManagerStatus-icon-' + item.value)
          statusKey.push('planManagerStatus-color-' + item.value)
        })
