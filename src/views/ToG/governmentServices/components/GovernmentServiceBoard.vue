<template>
  <main class="government-service-board">
    <section class="government-statistics" aria-label="政务服务统计">
      ...
      <article class="stat-card stat-card--process">
      ...
      <div v-if="records.length" class="government-grid">
        <article v-for="item in records" :key="item.id" class="government-card" tabindex="0" role="button" @click="openDetail(item)" @keydown.enter="openDetail(item)">
          <div class="card-head">
            ...
            <el-tag :type="statusType(statusValue(item))" size="small">{{ statusValue(item) }}</el-tag>
          </div>
          <div class="card-title">
            <i :class="config.icon"></i>
            <b>{{ item[config.primaryKey] || '-' }}</b>
            <time>{{ formatValue(item[config.timeKey]) }}</time>
          </div>
          <dl class="card-meta">
            <template v-for="field in cardFields">
              <dt :key="`${field.key}-dt`">{{ field.label }}</dt>
              <dd :key="`${field.key}-dd`">{{ formatValue(item[field.key]) }}</dd>
            </template>
          </dl>
          <div class="flow-steps">
            <span v-for="(step, index) in config.flow" :key="step" :class="{ active: index <= currentStep(item) }">
              <i>{{ index + 1 }}</i>
              {{ step }}
            </span>
          </div>
          <div class="card-foot">
            <div class="card-actions">
              ...
              <el-button v-if="canEdit(item)" type="text" size="mini" @click.stop="openEdit(item)">编辑</el-button>
              <el-button v-if="nextStatus(item)" type="text" size="mini" class="advance-action" @click.stop="advance(item)">推进至{{ nextStatus(item) }}</el-button>
              <el-button v-if="config.allowDelete && canDelete(item)" type="text" size="mini" class="danger-action" @click.stop="removeRecord(item)">删除</el-button>
            </div>
          </div>
        </article>
      </div>
    ...
    <el-dialog
      ...
      custom-class="government-form-dialog"
      @closed="resetForm">
        ...
        <el-row :gutter="28">
          <el-col v-for="field in normalFields" :key="field.key" :xs="24" :sm="12">
            <el-form-item :label="field.label" :prop="field.key">
              <el-select v-if="field.options" v-model="form[field.key]" clearable filterable class="field-full" :placeholder="`请选择${field.label}`">
          ...
          <el-col v-for="field in textFields" :key="field.key" :span="24">
            <el-form-item :label="field.label" :prop="field.key">
              <el-input v-model.trim="form[field.key]" type="textarea" :rows="5" :maxlength="1000" show-word-limit :placeholder="`请输入${field.label}`" />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="12">
            <el-form-item label="当前状态">
              <div class="readonly-status">
                <el-tag :type="statusType(form[config.statusKey])">{{ form[config.statusKey] }}</el-tag>
                <span>状态由办理流程推进</span>
              </div>
            </el-form-item>
          </el-col>
      ...
      <div v-if="selectedRecord" class="drawer-layout">
        <div class="drawer-scroll">
          <div class="detail-hero">
            <span><i :class="config.icon"></i></span>
            <div>
              <small>{{ selectedRecord.id }}</small>
              <h3>{{ selectedRecord[config.primaryKey] || '-' }}</h3>
            </div>
            <el-tag :type="statusType(statusValue(selectedRecord))">{{ statusValue(selectedRecord) }}</el-tag>
          </div>
          <div class="detail-flow">
            <span v-for="(step, index) in config.flow" :key="step" :class="{ active: index <= currentStep(selectedRecord) }">
              <i>{{ index + 1 }}</i>
              <b>{{ step }}</b>
            </span>
          </div>
...
export default {
  ...
  computed: {
    ...
    cardFields() {
      return this.fields.filter((field) => field.type !== 'textarea' && ![this.config.primaryKey, this.config.timeKey, this.config.statusKey].includes(field.key)).slice(0, 3)
    },
    rules() {
      const rules = {}
      this.fields
    ...
    pendingCount() {
      return this.records.filter((item) => this.currentStep(item) === 0).length
    },
    processingCount() {
      return this.records.filter((item) => this.currentStep(item) > 0 && this.currentStep(item) < this.config.flow.length - 1).length
    },
    completedCount() {
      return this.records.filter((item) => this.currentStep(item) === this.config.flow.length - 1).length
    }
  },
  created() {
    this.loadRecords()
  ...
  methods: {
    ...
    async loadRecords() {
      this.loading = true
      try {
        const params = { pageNo: this.currentPage, pageSize: this.pageSize, keyword: this.keyword || undefined, status: this.statusFilter || undefined }
        if (this.typeFilter) params[this.config.typeKey || this.config.primaryKey] = this.typeFilter
        ...
        if (!this.total) this.total = this.records.length
      } catch (error) {
        this.records = []
        this.total = 0
        this.$message.error(this.errorMessage(error, '列表加载失败，请稍后重试'))
      } finally {
        this.loading = false
      }
    },
    emptyForm() {
      ...
      form[this.config.statusKey] = this.config.statusOptions[0]
      return form
    },
    resetForm() {
      this.editingId = ''
      this.form = this.emptyForm()
      this.$nextTick(() => this.$refs.governmentForm && this.$refs.governmentForm.clearValidate())
    },
    openCreate() {
      this.editingId = ''
      this.form = this.emptyForm()
    ...
    openEdit(item) {
      ...
      this.form = Object.assign(this.emptyForm(), item)
      this.formVisible = true
    },
    async submitForm() {
      const valid = await new Promise((resolve) => this.$refs.governmentForm.validate(resolve))
      if (!valid) return
    ...
    statusValue(item) {
      return (item && item[this.config.statusKey]) || this.config.statusOptions[0]
    },
    currentStep(item) {
      const index = this.config.statusOptions.indexOf(this.statusValue(item))
      return index < 0 ? 0 : Math.min(index, this.config.flow.length - 1)
    },
    canEdit(item) {
      return this.currentStep(item) === 0
    },
    canDelete(item) {
      return this.currentStep(item) === 0
    },
    nextStatus(item) {
      const nextIndex = this.currentStep(item) + 1
      return this.config.statusOptions[nextIndex] || ''
    },
    statusType(status) {
      if (/(已办结|已归档|已完成)/.test(status)) return 'success'
      if (/(待|退回)/.test(status)) return 'warning'
      if (/(关闭|取消)/.test(status)) return 'info'
      return 'primary'
    },
    formatValue(value) {
    ...
    async advance(item) {
      ...
      try {
        ...
        if (error !== 'cancel') this.$message.error(this.errorMessage(error))
      }
    },
...
.government-surface {
  min-height: clamp(650px, calc(100vh - 256px), 790px);
  display: flex;
  flex-direction: column;
  border: 1px solid #e2e8f0;
  border-radius: 8px;
...
@media (max-width: 760px) {
  ...
  ::v-deep .government-detail-drawer {
    width: 100% !important;
    max-width: 100% !important;
  }
  ::v-deep .government-detail-drawer .el-drawer__body {
    display: flex;
    min-height: 0;
    flex: 1 1 auto;
    overflow: hidden;
  ...
  ::v-deep .government-form-dialog .el-dialog__body {
    min-height: 0;
    flex: 1 1 auto;
    overflow-y: auto;
    padding: 18px 16px;
  ...
  ::v-deep .government-form-dialog .el-dialog__footer {
    flex: 0 0 auto;
    padding: 12px 16px calc(12px + env(safe-area-inset-bottom));
  }
  ::v-deep .government-form-dialog .dialog-footer {
    display: flex;
    gap: 10px;
  }
  ::v-deep .government-form-dialog .dialog-footer .el-button {
    flex: 1;
    margin: 0;
    min-height: 42px;
  }
...
.government-service-board .stat-card {
  min-height: 94px;
  padding: 16px 18px;
  border-color: #e5ebf2;
  border-radius: 10px;
...
.government-service-board .list-toolbar {
  min-height: 64px;
  padding: 14px 20px;
  gap: 10px;
}
.government-service-board .list-toolbar .el-input {
  width: 300px;
}
.government-service-board .list-toolbar .el-select {
  width: 160px;
}
.government-grid {
  gap: 14px;
...
.government-service-board .empty-state {
  min-height: 260px;
  padding: 24px 20px 48px;
  box-sizing: border-box;
}
.government-service-board .pagination-row {
  min-height: 0;
  margin-top: auto;
