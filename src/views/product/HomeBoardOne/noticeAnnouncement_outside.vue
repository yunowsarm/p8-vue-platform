...
<template>
  <section class="notice-announcement" aria-label="通知公告">
    ...
    <el-drawer title="通知公告详情" :visible.sync="detailVisible" size="500px" append-to-body custom-class="home-notice-detail">
      <article v-if="selectedNotice" v-loading="detailLoading" class="notice-detail">
  ...
  methods: {
    ...
    async loadNotices() {
      ...
      try {
        ...
        this.notices = this.recordsFrom(result)
          .map((notice) => this.normalizeNotice(notice))
          .slice(0, 5)
      } catch (error) {
        this.notices = []
      } finally {
    ...
    async openDetail(notice) {
      this.selectedNotice = this.normalizeNotice(notice)
      ...
      try {
        ...
        if (detail) this.selectedNotice = this.normalizeNotice(Object.assign({}, notice, detail))
      } catch (error) {
        // 列表数据足以支撑基础详情回显。
      } finally {
    ...
    normalizeUploadFiles(value) {
      ...
      return files.filter(Boolean).map((item, index) => {
        ...
        return Object.assign({}, file, {
          ...
          uid: file.uid || file.attachmentId || file.id || file.fileId || filePath || `home-notice-file-${index}`,
    ...
    normalizeNotice(notice) {
      const normalized = Object.assign({}, notice || {})
      const status = Number(normalized.status)
      normalized.status = [0, 1].includes(status) ? status : Number(this.noticeType) || 0
      normalized.uploadFiles = this.normalizeUploadFiles(normalized.uploadFiles)
      return normalized
    },
    attachmentId(file) {
    ...
    async downloadUploadFile(file) {
      ...
      if (!attachmentId || !this.$api || !this.$api['attachment.download']) return
      try {
        const response = await this.$api['attachment.download']({ attachmentId }, { responseType: 'blob' })
