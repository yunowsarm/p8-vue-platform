...
export default {
  ...
  computed: {
    ...
    detailFiles() {
      ...
      let files = this.selectedRecord.uploadFiles
      ...
      return (Array.isArray(files) ? files : []).map((file) => (typeof file === 'string' ? { name: file.split('/').pop(), url: file } : file))
    }
  },
  created() {
    this.loadRecords()
  ...
  methods: {
    async loadRecords() {
      try {
        ...
        this.readRecords = records.filter((item) => this.statusMatches(item.status, 3))
      }
    },
    ...
    statusMatches(value, status) {
      const aliases = String(status) === '0' ? ['0', '待查阅'] : ['1', '已查阅']
    ...
    async markAsRead(item) {
      ...
      try {
        await editApi(Object.assign({}, item, { status: 1, updateBy: this.currentUserId(), itemUpdateTime: this.now() }))
