              ...
              <p>
                ...
                <span class="filename">
                  <el-link type="success"
                           :underline="true"
                           @click="downLoadMyList">柱网信息管理模板</el-link>
                </span>
              </p>
            </li>
          </ul>
...
export default {
  ...
  methods: {
    ...
    uploadExcelFile(item) {
      ...
      axios.post(CONFIG.devBaseUrl + '/jt/grid/import',formData,{
        headers: { 
    ...
    downLoadMyList () {
      this.$api['baseData.griddownload']({ url: "gridTemplate.xlsx" }, { responseType: 'blob' })
      .then((data) => {
          const date = new Date()
          const fileName = '柱网信息管理模板' + date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate()
          const fileType = 'xlsx'
