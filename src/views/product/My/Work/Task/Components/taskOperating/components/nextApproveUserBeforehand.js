export const nextApproveUserBeforehand = {
  ...
  initDataSource: function (processDefinitionKey, vueInstance) {
    ...
    return new Promise((resolve, reject) => {
      ...
      vueInstance.$api['PersonalProcessApproval.getTaskDefinition']({ processDefinitionKey: processDefinitionKey, businessIds: businessIdList })
        .then((res) => {
          if (res.processMode && res.processMode == 'AUTOMATIC') {
            vueInstance.AUTOMATIC = true
            resolve(true)
            return
          }
