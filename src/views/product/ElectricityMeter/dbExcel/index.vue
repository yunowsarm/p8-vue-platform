              ...
              <p>
                ...
                <span class="filename">
                  <el-link type="success"
                           :underline="true"
                           @click="downLoadMyList">电表信息导入模板</el-link>
                </span>
              </p>
            </li>
          </ul>
...
export default {
  ...
  methods: {
    ...
    uploadExcelFile(item) {
      ...
      axios.post(CONFIG.devBaseUrl + '/jt/electricity/importMeter',formData,{
        headers: { 
    ...
    downLoadMyList () {
      this.$api['baseData.griddownload']({ url: "electricityMeter.xlsx" }, { responseType: 'blob' })
      .then((data) => {
          const date = new Date()
          const fileName = '电表信息导入模板' + date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate()
          const fileType = 'xlsx'
