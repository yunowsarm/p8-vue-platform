<template>
  <div class="ai-chat">
    ...
    <div class="chat-window" ref="chatWindow" @scroll="handleScroll">
      ...
      <div v-for="(message, index) in messages" :key="index">
        <div v-if="!!message.content" :class="['message', message.sendUser === userId ? 'user' : 'ai']">
          <el-button v-if="message.styleType" class="task_preview" type="text" @click="teamPreview(message.content)">点击查看数据 </el-button>
    ...
    <common-dialog
      title="预览"
      width="60%"
      v-if="teamPreviewVisible"
      :visible="teamPreviewVisible"
      :show-handle-btn="false"
      @close="teamPreviewVisible = false"
      :is-view-cs-footer="false"
      :dialog-height="700"
    >
      <template #dialog>
        <team-preview v-if="teamPreviewVisible" :team-data="teamPreviewData" @success="addCurrentTeam"></team-preview>
      </template>
    </common-dialog>
  </div>
</template>
...
import TeamPreview from '@/views/product/Resource/Components/AiGeneratedDialog/TeamPreview'
...
export default {
  ...
  mounted() {
    window.myWebSocket.on('getIntelligenceMessage', (data) => {
      if (data.entityId === this.projectId) {
  ...
  methods: {
    ...
    sendMessage() {
      ...
      this.$api['teamManager.createTeamFromDescribe']({ wholeDescribeId: this.projectId, describe: this.userInput }).finally(() => {
        this.userInput = ''
        this.scrollToBottom()
      })
    },
    teamPreview(content) {
      this.teamPreviewData = JSON.parse(content)
      this.teamPreviewVisible = true
    },
    addCurrentTeam() {
