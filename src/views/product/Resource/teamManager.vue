    ...
    <div style="height: 100%" class="team-manager-wrap">
      <div class="custom_content_wrap">
        <div class="left_content">
          ...
          <div class="left_bottom_content">
            <div class="role-con">
              <el-button type="plan" style="margin: 5px; width: 90%" @click="addRolesHandle"><i class="el-icon-plus"></i> 新建角色类别 </el-button>
              <vue-perfect-scrollbar class="role-list">
                <li style="padding-left: 26px; color: #323232; font-size: 12px" :class="[{ active: rolesSelectedIndex === -1 }]" @click="refreshHandle">
                  所有人员<span>({{ getTotalCount }})</span>
                </li>
                <li
                  :class="[{ active: index === rolesSelectedIndex }, { 'fixed-role': item.roleType === 'fixed' }]"
            ...
            <div class="table-con">
              <div class="add-member">
                <div>
                  <el-button icon="el-icon-plus" v-if="group_add_member" type="plan" @click="addMemberHandle">添加人员 </el-button>
                  <!-- <el-button type="plan"
                           @click="refreshHandle">查看全部成员
                </el-button> -->
                </div>
                <search-form-list style="top: 2px" ref="search" :resetAfterToSearch="false" :dataSource="dataSource" @search="search" @re-set="reset"></search-form-list>
              </div>
                ...
                >
                  ...
                  <template #userState="{ scope }">
                    <div class="userState">
                      ...
                      <span v-if="scope.row.entryTime && scope.row.departureTime && scope.row.waitout" class="state-waitout">待退出</span>
                      <span v-if="scope.row.entryTime && scope.row.departureTime && !scope.row.waitout" class="state-out">已退出</span>
                    </div>
                  </template>
                  <template #operation="{ scope }">
                    <template v-if="!scope.row.departureTime">
                      <template v-if="scope.row.taskCount">
                        <el-popconfirm title="确认要将该人退出项目组吗?" confirmButtonText="确认" cancelButtonText="取消" @confirm="deleteUserHandle(scope, scope.$index)">
                          <el-button slot="reference" size="mini" type="text" v-if="group_add_member">删除 </el-button>
                        </el-popconfirm>
                      </template>
                      <template v-else>
                        <el-button v-if="group_add_member" size="mini" type="text" @click="deleteUserHandle(scope, scope.$index)">删除 </el-button>
                      </template>
                    </template>
                  </template>
      ...
      <div class="bottom-con">
        ...
        <p class="submit" v-if="group_add_role || group_add_member">
          ...
          <el-button size="mini" v-if="$route.name == 'ProjectInitiation'" type="primary" @click="saveAndRelease">发 布 </el-button>
        </p>
      </div>
      <dialog-tabs-roles
