...
<template>
  <div style="height: 100%; position: relative"
    ...
    <common-dialog title="任务列表"
                   width="40%"
                   v-if="ganttTaskVisible"
                   :visible="ganttTaskVisible"
                   :show-handle-btn="false"
                   @isfullscreen="isfullscreen"
                   @close="closeTaskVisible"
                   :is-view-cs-footer="false"
                   :dialog-height="460">
      <template #dialog>
...
import { taskStatisticsGantt } from './js/taskStatisticsGantt'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      viewType: 'resource', // 视图类型，默认只显示列表
      ...
      ganttTaskVisible: false, //  版本列表显示隐藏
      taskColumns,
      taskList: [],
      extraMap: {}
    }
  ...
  methods: {
    taskClick (data) {
      ...
      console.log("🚀 ~ taskClick ~ tasks:", tasks)
      this.ganttTaskVisible = true
      tasks.forEach(item => {
        let task = myGantt.getTask(item)
        this.taskList.push(task)
      })
      console.log("🚀 ~ taskClick ~ this.taskList:", this.taskList)
    },
    closeTaskVisible () {
      this.ganttTaskVisible = false
    },
    excelExport () {
    ...
    async initGantt (projectId, viewType) {
      ...
      myGantt.templates.resource_cell_value = function (start_date, end_date, res, tasks) {
        let ids = tasks.map(item => item.id).join(',')
        let html = `<div onclick="taskClick('${ids}')">${tasks.length}</div>`
        return html
      }
    ...
    loadGanttData (projectId) {
      ...
      const vueThis = this
      vueThis.$api['planGanttManager.loadProPlanGanttData']({
        dicType: 'ACTIVITY_TYPE',
        createPage: this.createPage
      })
        .then(function (res) {
