<template>
  <list-layout>
    <template #north>
      ...
      <span style="color: red; font-size: 14px; font-weight: bolder; float: right; line-height: 30px; display: inline-block; margin-right: 10px">密级：{{ courtyardSecretGradeDisplay }}</span>
    </template>
    <template #center>
...
export default {
  ...
  methods: {
    save() {
      ...
      const ids = this.selectedRows.map((i) => i.taskId)
      this.$api['myCourtyardTask.importCourtyardTask']({
        parentTaskId: this.tasks[0].id,
        planInfoId: this.tasks[0].planInfoId,
        experienceInfoIds: ids,
    ...
    handleSelectionChange(rows) {
      // if (rows.length >= 2) {
      //   let arrays = rows.splice(0, rows.length - 1)
      //   arrays.forEach(row => {
