...
<template>
  <div style="height: 100%; position: relative" class="Gantt">
    <div id="actionMenu" v-show="menuVisible && menuData.length" ref="actionMenu"
      ...
      <VuePerfectScrollbar class="scroll-area" :style="{ maxHeight: maxHeight, height: scrollBarHeight }">
        <el-menu mode="vertical" :collapse="true">
          <template v-for="(menu, index) in menuData">
            <el-submenu v-if="buttonData(menu).children" :disabled="isDisable(menu)" :key="menu.id"
              :index="index + 'm'">
              <span slot="title">
              ...
              <template v-for="(btn, index) in buttonData(menu).children">
                <el-menu-item v-if="btn.id !== 'createByNum'" :key="index"
                  @click="btnClick(btn, btn.isDisableFun(null, ganttName, selectedTasks))" :index="btn.id">
                  <i :class="btn.icon"></i>
                  <span> {{ btn.title }}</span>
                </el-menu-item>
                <el-submenu v-if="btn.id === 'createByNum'" :key="index + 'c'" :index="index + 'b'">
                  ...
                  <el-input-number size="mini" v-model="createNum" :max="50" :min="1" :step-strictly="true"
                    style="margin: 0 10px" :step="1"></el-input-number>
                  <el-button size="mini" @click="btn.clickFun(btn, ganttName, null)"
                    style="margin-right: 10px">确定</el-button>
                </el-submenu>
              </template>
            </el-submenu>
            <el-menu-item v-else @click="btnClick(buttonData(menu), isDisable(menu))" :disabled="isDisable(menu)"
              :key="menu.id" :index="menu.id + 'm'">
              <i :class="buttonData(menu).icon"></i>
              <span> {{ menu.title }}</span>
            </el-menu-item>
          </template>
    ...
    <out-put-view v-if="outPutViewVisible" :visible="outPutViewVisible" :plan-info-id="planInfoId" :task-id="taskId"
      :create-page="createPage" :gantt-name="ganttName" :plan-type-datas="planTypeDatas"
      ...
      <div style="width: 50%">
        ...
        <el-popover placement="top" trigger="click">
          <div class="edit_gantt_user_list">
            ...
            <span v-else v-for="(user, ind) in editUserList" :key="ind">{{ user.userName }}
              <span v-if="user.entityType === 'compile'">-计划编制</span>
              <span v-if="user.entityType === 'decompose'">-任务分解</span>
            </span>
          </div>
          <span slot="reference" style="float: right; margin-right: 40px; cursor: pointer">正在编辑 {{ webSocketDone ? '*' :
        editUserList.length == 0 ? 1 : editUserList.length }} 人</span>
        </el-popover>
      </div>
    </div>
    <el-drawer :title="activityImportTitle" :append-to-body="true" size="50%" :destroy-on-close="true"
      :wrapper-closable="false" @closed="activityImportVisible = false" :visible.sync="activityImportVisible">
      <activity-import @save-success="activityImportClosed" :task-id="selectTaskId"
        :activity-secret-grade-display="activitySecretGradeDisplay" :create-page="createPage"
        :activity-import-type="activityImportType" :auto-scheduling="autoParentDate"></activity-import>
    </el-drawer>
    <common-dialog title="通知下发" width="70%" :visible="noticeVisible" :show-handle-btn="false"
      @isfullscreen="isfullscreen" @close="closeNotice" :is-view-cs-footer="false" :dialog-height="650">
      <template #dialog>
        <Notice v-if="noticeVisible" :selected-tasks="selectedTasks" :task-id="selectTaskId" :gantt-name="ganttName"
          :plan-info-id="planInfoId" @close="closeNotice" />
      </template>
    </common-dialog>
    <!-- ai自动生成   -->
    <common-dialog title="AI生成" width="50%" class='autoGeneration' :visible="autoGenerationVisible"
      @close="closeAutoGeneration" :is-view-cs-footer="false" :dialog-height="650" :show-handle-btn="false">
      <template #dialog>
        <auto-generation v-if="autoGenerationVisible" :selected-tasks="selectedTasks" ref="autoGeneration"
          :task-id="selectTaskId" :gantt-name="ganttName" :plan-info-id="planInfoId" @refreshAiData="refreshAiData"
          @close="closeAutoGeneration" />
      </template>
    </common-dialog>
    <monitor-time-manger v-if="controlTimeVisible" :visible="controlTimeVisible" :monitor-id="monitorId"
      :task-id="selectTaskId" :monitor-name="monitorName" :task-name="selectTaskName"
      @save-success="monitorManagerSave">
    </monitor-time-manger>
    <resource-select v-if="resourceSelectVisible" :visible="resourceSelectVisible" :start-task-id="startTaskId"
      :end-task-id="endTaskId" :plan-info-id="planInfoId" :select-task-owner-id="selectTaskOwnerId"
      :select-model="resourceSelectModel" @closed="resourceSelectclosed" @resource-selected="resourceSelected">
    </resource-select>
    <grid-setting v-if="selectGridVisible" :visible="selectGridVisible" :columns="renderColumns" :gantt-name="ganttName"
      :create-page="createPage" @close="selectGridlosed" @save-setting="gridSaved"></grid-setting>
    <common-dialog title="请选择需要保存的任务" :visible="myExperienceVisible" :show-handle-btn="false"
      @isfullscreen="isfullscreen" @close="closeMyExperience" :is-view-cs-footer="true">
      <template #dialog>
        <list-layout>
          <template #north>
          ...
          <template #center>
            <common-table ref="table" v-if="myExperienceVisible" :comp="comp" :style="{ height: customHeight + 'px' }"
              :table-config="tableConfig" :columns="Mycolumns" :api="tableApi" :params="queryParam"
              :special-rote-name="roteName" :use-tree-format="useTreeFormat" :use-tree-p-id="useTreePId"
              :pagination="false" @select="onTableSelect" @select-all="selectAll"
    ...
    <el-drawer :title="importExcelTitle" :append-to-body="true" size="50%" :wrapper-closable="false"
      :destroy-on-close="true" @closed="importExcelClosed" :visible.sync="importExcel">
      <import-excel @save-success="importExcelSave" @excel-export="excelExport"
        :excel-secret-grade-display="excelSecretGradeDisplay" :excel-secret-grade="excelSecretGrade"
        :task-id="selectTaskId" :planInfoId="planInfoId" :columnConfigs="columnConfigs"
        :output-request="excelImportData"></import-excel>
    </el-drawer>
    <el-drawer :title="importProjectTitle" :append-to-body="true" size="50%" :wrapper-closable="false"
      :destroy-on-close="true" @closed="resourceSettingClosed" :visible.sync="importProject">
      <import-project @save-success="importProjectClosed" :project-secret-grade-display="projectSecretGradeDisplay"
        :project-secret-grade="projectSecretGrade" :task-id="selectTaskId"
        :output-request="projectImportData"></import-project>
    </el-drawer>
    <!--    团队成员编辑-->
    <el-drawer class="resource-group-drawer" :title="resourceGroup" :append-to-body="true"
      :before-close="isSaveCheckHandle" size="100%" :destroy-on-close="true" @closed="importProjectClosed"
    ...
    <common-dialog title="查询" width="90%" v-if="ganttSearchVisible" :visible="ganttSearchVisible"
      :show-handle-btn="false" @isfullscreen="isfullscreen" @close="closeSearch" :is-view-cs-footer="false"
      :dialog-height="360">
      <template #dialog>
        <command-search :searchType="searchType" :gantt-name="ganttName" :plan-info-id="planInfoId"
          @close="closeSearch"></command-search>
      </template>
    </common-dialog>
    <common-dialog title="统计信息" width="60%" v-if="ganttStatisticVisible" :visible="ganttStatisticVisible"
      :show-handle-btn="false" @isfullscreen="isfullscreen" @close="closeStatistic" :is-view-cs-footer="false"
      :dialog-height="460">
      <template #dialog>
        <command-statistic :gantt-name="ganttName" :plan-info-id="planInfoId"></command-statistic>
      </template>
    </common-dialog>
    <common-button-bar-setting v-if="rightMenuConfigVisible" :visible="rightMenuConfigVisible" title="菜单配置"
      :panel-data="panelData" @submit="submitButtonBarSetting" @hidden="rightMenuConfigVisible = false">
    ...
    ></ClassificationSelection> -->
    <Edit :visible="createVisible" :selected-task="selectedTasks" title="我的经验库" :gantt-name="ganttName"
      @handleCancel="closeCreate" />
    <my-experience-base :visible="experienceBaseVisible" v-if="experienceBaseVisible" :is-manage="isManage"
      :gantt-name="ganttName" :create-page="createPage" :plan-info-id="planInfoId" :selected-task="selectedTasks"
      :thirdMenuParam="thirdMenuParam" :export-experience-type="exportExperienceType" @copy="copyExperienceBase"
      @handleCancel="closExperienceBase"></my-experience-base>
    <common-drawer v-if="versionListVisible" :visible="versionListVisible" size="70%" placement="top" title="版本列表"
    ...
    <common-drawer v-if="changeHistoryVisible" :visible="changeHistoryVisible" size="80%" placement="top" title="任务历史变更"
      @close="changeHistoryClose">
      <template #drawer>
        <ChangeHistory :plan-info-id="planInfoId" :task-id="selectTaskId" :wholeDescribeId="wholeDescribeId"
          :create-page="createPage" />
      </template>
    </common-drawer>
    <common-drawer v-if="examineHistoryVisible" :visible="examineHistoryVisible" size="80%" placement="top" title="审批历史"
      @close="examineHistoryClose">
      <template #drawer>
        <ExamineHistory :plan-info-id="planInfoId" :task-id="selectTaskId" :create-page="createPage" />
      </template>
    </common-drawer>
    <common-drawer v-if="relevancePlanVisible" :visible="relevancePlanVisible" size="100%" placement="top" title="关联"
      @close="closeRelevance">
      <template #drawer>
        <relevance :plan-info-id="planInfoId" ref="relevanceRefs" @relevanceClick="relevanceClick"
          :selectTaskId="selectTaskId" :main-gantt-name="ganttName"></relevance>
      </template>
    </common-drawer>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    excelExport() {
      ...
      vueThis.$api['planGanttManager.excelExport'](exportConfig, { responseType: 'blob' })
        .then((data) => {
          ...
          const file_name = `【计划编制数据导出】${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}_${String(date.getHours()).padStart(2, '0')}h${String(date.getMinutes()).padStart(
            2,
            '0'
          )}m${String(date.getSeconds()).padStart(2, '0')}s`
          // eslint-disable-next-line camelcase
          const file_type = 'xls'
    ...
    closExperienceBase(data) {
      this.isManage = false
      this.experienceBaseVisible = false
      if (data && data.length > 0) {
        this.loadGanttData(this.planInfoId, this.taskId, this.createPage).then(() => {
          data.forEach((item) => {
            myGantt.selectTask(item)
            myGantt.showTask(item)
          })
        })
      }
    },
    ...
    activityImportClosed(data) {
      this.activityImportVisible = false
      // await this.callParentSelectTasks()
      if (data && data.length > 0) {
        this.loadGanttData(this.planInfoId, this.taskId, this.createPage).then(() => {
          data.forEach((item) => {
            myGantt.selectTask(item)
