...
export default {
  ...
  methods: {
    ...
    customValidate (saveParams) {
      ...
      if (Number(this.projectSecretGrade) < Number(params.uploadFileJson[0].attConfidentialite)) {
        this.$message.warning('文件密级高于计划密级!')
      } else {
        this.$api['planGanttManager.importProjectCheckout'](params)
          .then((data) => {
            if (data.length > 0) {
              let messages = '<ol> '
