              ...
              <p>
                ...
                <span class="filename">
                  <el-link type="success"
                           :underline="true"
                           @click="downLoadMyList">水表信息导入模板</el-link>
                </span>
              </p>
            </li>
          </ul>
...
export default {
  ...
  data () {
    ...
    return {
      saveApi: 'baseData.importwaterMeter',
      dataSource,
  ...
  methods: {
    ...
    uploadExcelFile(item) {
      ...
      axios.post(CONFIG.devBaseUrl + '/jt/water/importMeter',formData,{
        headers: { 
    ...
    downLoadMyList () {
      this.$api['baseData.griddownload']({ url: "waterMeter.xlsx" }, { responseType: 'blob' })
      .then((data) => {
          const date = new Date()
          const fileName = '水表信息导入模板' + date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate()
          const fileType = 'xlsx'
