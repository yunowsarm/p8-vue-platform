...
export default {
  ...
  computed: {
    ...
    summaryCards() {
      ...
      return definitions.map((item) =>
        Object.assign({}, item, { value: item.all ? this.records.length : item.page ? this.pagedRecords.length : item.value !== undefined ? item.value : count(item.statuses || []) })
      )
    },
    rules() {
      const rules = {}
  ...
  methods: {
    ...
    canChangeRecordStatus(record) {
      return this.canChangeStatus && this.nextStatusOptions(record).length > 0
    },
