...
const views = {
  ...
  mutations: {
    ...
    ADD_VISITED_VIEW(state, { route, token }) {
      if (state.visitedViews.some((v) => v.path === route.path) || route.matched.length === 3) return
    ...
    REMOVE_OTHER_VIEW(state, route) {
      state.visitedViews = state.visitedViews.filter((r) => {
        return r.name === store.getters.homepage.name || r.path === route.path
      })
      state.cachedViews = _initVisitedViews(state)
    },
    // 移除当前标签右侧的所有标签页
    REMOVE_RIGHT_VIEW(state, route) {
      ...
      state.visitedViews = state.visitedViews.filter((view, index) => index <= idx || view.name === store.getters.homepage.name)
      state.cachedViews = _initVisitedViews(state)
    },
    //
