<template>
  <div class="card-container" v-if="panelData.length > 1">
    <el-tabs type="border-card" v-if="advance">
      <el-tab-pane
      ...
      >
        <div v-if="isExecute(panel.groups)" class="group-search">
          <slot :name="panel.groups" :row-num="rowNum"></slot>
          <!-- <component v-bind="$attrs" :is="panel.groups" :gantt-object="ganttObject" :plan-info-id="planInfoId" :row-num="rowNum"></component> -->
        </div>
        <div v-else class="groupContain" :class="{ taskGroupContain: index === 0 }">
          <div class="group" v-for="(group, index1) in panel.groups" :key="index1">
            <div v-if="isExecute(group.buttonConfigs)" class="group-container">
              <slot :name="group.buttonConfigs" :row-num="rowNum"></slot>
              <!-- <component v-bind="$attrs" :is="" :select-tasks="selectedTasks" :gantt-object="ganttObject" :plan-info-id="planInfoId" :row-num="rowNum"></component> -->
            </div>
            <div v-else class="group-container">
              <template v-for="(childGroup, idx) in childGroups(group.buttonConfigs)">
                <div class="child-group" :key="idx">
                  <command-button
                    v-for="(config, ids) in childGroup.configs"
                    :key="ids"
                    :cbutton="buttonData(config)"
                    :size="config.size"
                    :select-tasks="selectedTasks"
                    :gantt-object="ganttObject"
                  ></command-button>
                </div>
              </template>
            </div>
  ...
  <div class="card-container" :class="{ double: ganttButtonMode === 'tabs', double: ganttButtonMode === 'double', single: ganttButtonMode === 'single' }" v-else>
        ...
        <div class="group" v-for="(group, index) in panelData[0].groups" :key="index">
          <div v-if="isExecute(group.buttonConfigs)" class="group-container">
            <div class="group-icon">
              <slot :name="group.buttonConfigs" :row-num="rowNum"></slot>
              <!-- <component v-bind="$attrs" :is="group.buttonConfigs" :select-tasks="selectedTasks" :gantt-object="ganttObject" :plan-info-id="planInfoId" :row-num="rowNum"></component> -->
            </div>
          ...
          <div v-else class="group-container">
            <div class="group-icon">
              <template v-for="(childGroup, idx) in childGroups(group.buttonConfigs)">
                <div class="child-group" :key="idx">
                  <span v-for="(config, ids) in childGroup.configs" :key="ids">
                    <command-button
...
import CommandButton from '../GanttButton/index.vue'
...
export default {
  ...
  data() {
    return {
      ...
      leftNoAction: true,
      rightNoAction: false,
      rowNum: this.ganttButtonMode === 'single' ? 1 : 2,
