...
import { Link, Notification } from 'element-ui'
export default {
  name: 'ImportExcel',
  components: {
    FormList,
  ...
  methods: {
    ...
    getOutputIoData() {
      const _this = this
      this.$api[this.saveApi]({ taskId: _this.taskId }).then((res) => {
    ...
    customValidate(saveParams) {
      if (!this.checkOutApi || !this.saveApi) {
        this.$message.warning('请先配置表格上传所需要的api')
        return
      }
      ...
      this.$api[this.checkOutApi](params)
        .then((data) => {
    ...
    downloadOutputRequsetFile(item) {
      // 输出要求-文件下载
      if (item.attId) {
        this.$api['SystemSettings.getFileUrl']({ attachmentId: item.path }, { responseType: 'blob' })
          .then((backJson) => {
