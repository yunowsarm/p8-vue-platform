  ...
  <div>
    <template v-for="(mon, index) in childGroups(classifyData)">
      <div class="child-group" :key="'classify' + index">
        <command-button
          v-for="(config, index) in mon.configs"
          :key="config.id"
    ...
    isDisableFun() {
      ...
      return (btn, ganttName, tasks) => {
        ...
        if (!hasIsLeaf && btn.editMark == '1') {
          that.$store.commit('SET_BUTTONMSG', { id: btn.id, msg: '非叶子节点不允许此操作' })
          return true
        }
        ...
        if (isDisableRes) {
          that.$store.commit('SET_BUTTONMSG', { id: btn.id, msg: isDisableRes.tip })
          return true
        } else {
          return false
        }
