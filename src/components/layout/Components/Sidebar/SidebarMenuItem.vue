        ...
        <el-submenu v-else :index="item.name || item.path" :key="item.name">
          ...
          <template v-for="child in item.children">
            <template v-if="!child.hidden">
              <sidebar-menu-item v-if="child.children && child.children.length > 0" :is-nest="true" class="nest-menu" :routes="[child]" :key="child.path"></sidebar-menu-item>
              <template v-else>
...
<script>
import { MenuItem, Submenu } from 'p8-components-ui'
