...
export const CommandButtonData = [
  ...
  {
    id: 'research-task-relevance',
    icon: 'p8 icon-can-commit',
    title: '院任务关联',
    help: '院任务关联',
    clickFun: function (btn, ganttName, tasks) {
      const vueThis = store.getters.vueThis
      vueThis.researchTaskRelevanceShow = true
    },
    isDisableFun: function (btn, ganttName, tasks) {
      let result
      if (tasks && tasks.length > 0) {
        const check = tasks.some((item) => item.monitorPoints.includes('1023'))
        if (check) {
          return true
        }
        if (!tasks[0].parent) {
        ...
        if (tasks[0].managerStatus === '6409' || tasks[0].managerStatus === '6406') {
          result = true
        } else {
          result = false
        }
    ...
    clickFun: function (btn, ganttName, tasks) {
      ...
      axios
        ...
        .then((res) => {
          ...
          // 更新vuex
          store.commit('SET_SETTING_ALL', res)
        })
        .catch((err) => {
