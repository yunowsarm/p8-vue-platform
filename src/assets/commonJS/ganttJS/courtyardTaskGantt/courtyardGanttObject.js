...
import { getGanttColumns } from './courtyardTaskGanttObject'
 ...
 */
export const GanttCourtyardObject = {}

/**
 * 预计时间与计划开始时间不一致时，预计时间颜色定义
 * @type {string}
 */
GanttCourtyardObject.forecastColor = '#ff0000'
 ...
 */
GanttCourtyardObject.resourceOnAfterSelect = function (ganttObject) {
 ...
 */
GanttCourtyardObject.grid_row_class = function (ganttObject) {
 ...
 */
GanttCourtyardObject.resource_cell_value = function () {
 ...
 */
GanttCourtyardObject.task_row_class = function (ganttObject) {
  return function (start, end, task) {
    if (shouldHighlightTask(task, ganttObject)) {
 ...
 * @date 2020/5/15 15:39
 */
GanttCourtyardObject.resource_cell_class = function () {
  // eslint-disable-next-line camelcase
  return function (start_date, end_date, resource, tasks) {
    let css = []
 ...
 */
GanttCourtyardObject.timeline_cell_class = function (ganttObject) {
 ...
 */
GanttCourtyardObject.resourceTemplates = function (ganttObject) {
  return {
 ...
 */
GanttCourtyardObject.resourceConfig = function (ganttObject, vueThis) {
  return {
    scale_height: 30,
    // scales: [
 ...
 * @date 2021/1/9 19:03
 */
GanttCourtyardObject.layout1_fun = (firstWidth = 600) => {
  return {
    css: 'gantt_container',
    cols: [
      {
 ...
 */
GanttCourtyardObject.layout3 = function (resourceConfig, resourceTemplates) {
  return {
    css: 'gantt_container',
    rows: [
      {
 ...
 */
GanttCourtyardObject.getGanttObject = function getGanttObject(name) {
  if (name.replace(/(^s*)|(s*$)/g, '').length > 0) {
    if (Object.keys(ganttObjectMap).length > 0 && ganttObjectMap[name] && Object.keys(ganttObjectMap[name]).length > 0) {
      ...
      const newObj = Gantt.getGanttInstance(GanttCourtyardObject.publicObject)
      // 本地化
...
GanttCourtyardObject.setGanttObject = function setGanttObject(name, ganttObject) {
  if (name.replace(/(^s*)|(s*$)/g, '').length > 0) {
 ...
 * @date 2020/5/22 9:04
 */
GanttCourtyardObject.getDpObject = function getDpObject(name) {
  if (name.replace(/(^s*)|(s*$)/g, '').length > 0) {
 ...
 * @date 2020/5/22 9:04
 */
GanttCourtyardObject.setDpObject = function setDpObject(name, dpObject) {
  if (name.replace(/(^s*)|(s*$)/g, '').length > 0) {
 ...
 * @constructor
 */
GanttCourtyardObject.dateToStr = function (date, format, ganttObject) {
  let formatFunc
  if (format) {
 ...
 * @constructor
 */
GanttCourtyardObject.strToDate = function (date, format, ganttObject) {
  let formatFunc
  if (format) {
 ...
 * @date 2020/5/8 19:05
 */
GanttCourtyardObject.getActions = function (ganttObject) {
  return {
    undo: function () {
    ...
    indentAction: function indent(taskId) {
      ...
      if (prevId) {
        ...
        GanttCourtyardObject.calculateForecastDate(ganttObject, newParent)
    ...
    outdentAction: function outdent(taskId, initialIndexes, initialSiblings) {
      ...
      if (ganttObject.isTaskExists(oldParent.id) && oldParent.id !== ganttObject.config.root_id && oldParenrPar !== ganttObject.config.root_id) {
        ...
        ganttObject.updateTask(taskId)
        GanttCourtyardObject.calculateForecastDate(ganttObject, oldParent)
        // 旧父排程为自动时进度计算
        if (oldParent.autoScheduling === '1') {
          oldParent.progress = GanttCourtyardObject.calculateProgress(oldParent, ganttObject)
        }
 ...
 * @date 2020/5/9 11:31
 */
GanttCourtyardObject.performAction = function (actions, ganttObject) {
 ...
 */
GanttCourtyardObject.autoScheduleList = function (ganttObject) {
 ...
 */
GanttCourtyardObject.formatter = function (ganttObject) {
 ...
 */
GanttCourtyardObject.linksFormatter = function (ganttObject, formatter) {
...
GanttCourtyardObject.endDateEditor = function (ganttObject) {
 ...
 */
GanttCourtyardObject.editors = function (ganttObject, formatter, linksFormatter) {
  return {
    feedbackMsg: { type: 'text', map_to: 'feedbackMsg' },
 ...
 */
GanttCourtyardObject.updateScheduling = function (ganttObject, vueThis) {
 ...
 */
GanttCourtyardObject.unMoveTask = function (vueThis, ganttObject) {
 ...
 * @date 2020/5/14 10:30
 */
GanttCourtyardObject.changeUnMoveTask = function (vueThis, ganttObject) {
  return ganttObject.attachEvent('onBeforeRowDragEnd', function (id, parent, tindex) {
    return false
  })
}
 ...
 */
GanttCourtyardObject.calculateProgress = function calculateProgress(task, ganttObject) {
  let totalToDo = 0
  let totalDone = 0
 ...
 */
GanttCourtyardObject.refreshProgress = function refreshProgress(id, submit, ganttObject, vueThis) {
  if (!ganttObject.isTaskExists(id)) {
    return
  }
  let task = ganttObject.getTask(id)
  task.progress = GanttCourtyardObject.calculateProgress(task, ganttObject)
  if (submit) {
 ...
 */
GanttCourtyardObject.showMessage = function showMessage(vueThis, msg, type) {
  if (type) {
    if (type === 'error') {
 ...
 * @param arr2
 */
GanttCourtyardObject.calculateArrayContain = function calculateArrayContain(taskMonitors, selMonitor) {
...
function searchFilter(parent, searchForm, ganttObject) {
  if (JSON.stringify(searchForm) !== '{}') {
    ...
    let userName = searchForm.realName // 责任人模糊查询
    let userNameCheck = true

    if (userName && task.realName.indexOf(userName) === -1) {
      userNameCheck = false
    }

    let deptName = searchForm.dutyDeptName // 部门模糊查询
    let deptNameCheck = true
    if (deptName && task.dutyDeptName.indexOf(deptName) === -1) {
      deptNameCheck = false
    }
    ...
    if (monitors && monitors.length > 0 && !GanttCourtyardObject.calculateArrayContain(task.monitorPoints, monitors)) {
      monitorsCheck = false
    }
    let monitorPoints = searchForm.monitorPoints // 标识--列头
    if (monitorPoints && monitorPoints.length > 0 && !GanttCourtyardObject.calculateArrayContain(task.monitorPoints, monitorPoints)) {
      monitorsCheck = false
    }
 ...
 * @returns {*}
 */
GanttCourtyardObject.setSearchConfig = function (ganttObject, vueThis) {
 ...
 * @param ganttObject
 */
GanttCourtyardObject.createDatastore = function (ganttObject) {
 ...
 * @param ganttObject
 */
GanttCourtyardObject.resourceStoreOnParse = function (ganttObject) {
 ...
 * @param vueThis
 */
GanttCourtyardObject.createRightMenu = function (ganttObject, vueThis) {
 ...
 * @returns {*}
 */
GanttCourtyardObject.setOnBeforeEditStart = function (ganttObject, vueThis) {
  return ganttObject.ext.inlineEditors.attachEvent('onBeforeEditStart', function (state) {
    ...
    // 分配给当前责任人的节点，只能修改预计完成时间\预计开始时间\调整原因
    console.log(
      task.dutyUserId,
      task.nowUserId,
      'task.dutyUserId === task.nowUserIdtask.dutyUserId === task.nowUserIdtask.dutyUserId === task.nowUserIdtask.dutyUserId === task.nowUserIdtask.dutyUserId === task.nowUserIdtask.dutyUserId === task.nowUserId'
    )
    if (task.dutyUserId === task.nowUserId) {
      if (colName !== 'forecastBeginDate' && colName !== 'forecastEndDate' && colName !== 'feedbackMsg') {
        return false
      }
    } else {
      return false
 ...
 * @returns {*}
 */
GanttCourtyardObject.setCellSaveConfig = function (ganttObject) {
 ...
 * @returns {*}
 */
GanttCourtyardObject.checkoutBeforeCellSave = function (ganttObject, vueThis) {
  return ganttObject.ext.inlineEditors.attachEvent('onBeforeSave', (state) => {
        ...
        case 'duration':
          ...
          if (state.newValue === state.oldValue) {
            ...
            if (newVal > max || newVal < 1) {
              ...
              task.forecastBeginDate = GanttCourtyardObject.dateToStr(task.start_date, '%Y-%m-%d', ganttObject)
              task.forecastEndDate = GanttCourtyardObject.dateToStr(endDate, '%Y-%m-%d', ganttObject)
    ...
    // 错误时提醒
    if (!result && msg !== '') {
      GanttCourtyardObject.showMessage(vueThis, msg, 'error')
    }
    return result
  })
}
 ...
 * @returns {*}
 */
GanttCourtyardObject.onSaveCellEven = function (ganttObject, vueThis) {
  return ganttObject.ext.inlineEditors.attachEvent('onSave', (state) => {
    ...
    if (colName === 'end_date' || colName === 'duration' || colName === 'start_date' || colName === 'predecessors') {
      GanttCourtyardObject.updateTaskNew(ganttObject, taskId, vueThis)
    }
    // 同步左下角选中任务名称
    if (colName === 'name') {
 ...
 * @returns {*}
 */
GanttCourtyardObject.beforeLinkAddCheck = function (ganttObject) {
 ...
 * @returns {*}
 */
GanttCourtyardObject.beforeLinkUpdateCheck = function (ganttObject) {
...
/**
 * gantt图缩放（ctrl+鼠标滚轮），使用方式 thisGantt.ext.zoom.init(GanttCourtyardObject.zoomConfig(thisGantt))
 * 由于编辑视图不包含gantt图，无法使用
 * @param ganttObject
 */
GanttCourtyardObject.zoomConfig = function ganttZoomConfig(ganttObject) {
  return {
    // minColumnWidth: 20,
    // maxColumnWidth: 150,
    levels: [
 ...
 * @param ganttObject
 */
GanttCourtyardObject.workTimeSetting = function (ganttObject, vueThis) {
 ...
 */
GanttCourtyardObject.linkDescription = function (ganttObject) {
 ...
 * @param ganttObject
 */
GanttCourtyardObject.addTooltip = function (ganttObject, vueThis) {
  ganttObject.plugins({
    tooltip: true
  })
 ...
 * @date 2020/5/8 19:04
 */
GanttCourtyardObject.publicObject = {
  plugins: {
  ...
  config: {
    ...
    layout: GanttCourtyardObject.layout2, // 默认布局为只显示树编辑列表
 ...
 */
GanttCourtyardObject.updateTaskNew = function (ganttObject, taskId, vueThis) {
  ganttObject.batchUpdate(function () {
    let task = ganttObject.getTask(taskId)
 ...
 * @param vueThis
 */
GanttCourtyardObject.planChangeCheck = function (ganttObject, vueThis) {
 ...
 * @returns {*}
 */
GanttCourtyardObject.planAfterRedo = function (ganttObject, vueThis) {
 ...
 * @returns {*}
 */
GanttCourtyardObject.planAfterUndo = function (ganttObject, vueThis) {
 ...
 * @returns {*}
 */
GanttCourtyardObject.onBeforeUndoStack = function (ganttObject) {
  // 在将操作添加到撤消堆栈之前触发
 ...
 * @returns {*}
 */
GanttCourtyardObject.onBeforeRedoStack = function (ganttObject) {
  // 在将操作添加到回退堆栈之前触发
 ...
 * @returns {*}
 */
GanttCourtyardObject.onBeforeUndo = function (ganttObject) {
 ...
 * @returns {*}
 */
GanttCourtyardObject.onBeforeRedo = function (ganttObject) {
 ...
 * @returns {null|*}
 */
GanttCourtyardObject.getGanttSettingGrid = function (ganttName, createPage) {
 ...
 * @returns {[]}
 */
GanttCourtyardObject.synchronizationColumns = function (vueThis, ganttObject) {
  ...
  // 获取gantt列配置信息
  let ganttSetting = GanttCourtyardObject.getGanttSettingGrid(vueThis.ganttName, vueThis.createPage)
...
GanttCourtyardObject.searchColumnsDataInit = function (vueThis, ganttObject) {
 ...
 * @param task
 */
GanttCourtyardObject.calculateParentForecastDate = function (ganttObject, task) {
  if (task.parent) {
...
export function updateforecastDate(task, ganttObject) {
  task.forecastBeginDate = GanttCourtyardObject.dateToStr(task.start_date, '%Y-%m-%d', ganttObject)
  task.forecastEndDate = GanttCourtyardObject.dateToStr(ganttObject.date.add(task.end_date, -1, 'day'), '%Y-%m-%d', ganttObject)
  task.adjustForecastBeginDate = null
  task.adjustForecastEndDate = null
}
 ...
 * @param ganttObject
 */
GanttCourtyardObject.checkIsCriticalTask = function (ganttObject) {
 ...
 * @param task
 */
GanttCourtyardObject.calculateForecastDate = function (ganttObject, task) {
  // if (ganttObject.hasChild(task.id)) {
