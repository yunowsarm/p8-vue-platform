import { GanttCourtyardObject, progressRefreshCheck, updateforecastDate } from './courtyardGanttObject'
...
export const CommandButtonData = [
  ...
  },
  {
    ...
    help: '升级',
    clickFun: function (btn, ganttName, tasks) {
      let thisGantt = GanttCourtyardObject.getGanttObject(ganttName)
  ...
  },
  {
    ...
    clickFun: function (btn, ganttName, tasks) {
      ...
      if (ganttName === 'changeGantt') {
        ...
        let thisDp = GanttCourtyardObject.getDpObject(ganttName)
        if (thisGantt && thisDp) {
          let msg = '是否确认删除选中任务?'
...
function addTask(num, pos, ganttName) {
  let ganttObject = GanttCourtyardObject.getGanttObject(ganttName)
  let dpObject = GanttCourtyardObject.getDpObject(ganttName)
    ...
    ganttObject.batchUpdate(function () {
      allParTaskIds.forEach(function (id) {
        if (ganttObject.isTaskExists(id)) {
          ...
          parTask.progress = GanttCourtyardObject.calculateProgress(parTask, ganttObject)
            ...
            ganttObject.batchUpdate(function () {
              Array.from(new Set(allParTaskIds)).forEach(function (id) {
                if (ganttObject.isTaskExists(id)) {
                  let parTask = ganttObject.getTask(id)
                  parTask.progress = GanttCourtyardObject.calculateProgress(parTask, ganttObject)
                  GanttCourtyardObject.calculateForecastDate(ganttObject, parTask)
                  ganttObject.updateTask(id)
                }
              })
            })
