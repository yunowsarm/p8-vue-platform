...
GanttObject.refreshProgress = function refreshProgress(id, submit, ganttObject, vueThisLocation) {
  ...
  if (task.parent) {
    refreshProgress(task.parent, submit, ganttObject, vueThisLocation)
  }
}

/**
 * 通用消息提醒
 * @param vueThisLocation 展示页面this对象
 * @param msg 提示内容
...
GanttObject.showMessage = function showMessage(vueThisLocation, msg, type) {
  if (type) {
    if (type === 'error') {
      ...
      vueThisLocation.$message({
        message: msg,
        type: type
      })
    }
  } else {
    vueThisLocation.$message(msg)
  }
}
...
function searchFilterLocation(parent, searchForm, ganttObject) {
  ...
  if (keys.length > 0) {
    keys.forEach(el => {
      let value = searchForm[el]
      el = el.replace('Location', '')
      searchForm[el] = value
    })
  }
  const vueThisLocation = store.getters.vueThisLocation
  if (JSON.stringify(searchForm) !== '{}') {
    ...
    for (const val of child) {
      if (searchFilterLocation(val, searchForm, ganttObject)) {
        return true
      }
    }
    return false
...
GanttObject.resourceStoreOnParse = function (ganttObject) {
  return ganttObject.$resourcesStore.attachEvent('onParse', function () {
    ...
    const vueThisLocation = store.getters.vueThisLocation
    if (vueThisLocation.viewType === 'resource') {
...
GanttObject.createRightMenu = function (ganttObject, vueThisLocation) {
  return ganttObject.attachEvent('onContextMenu', function (taskId, linkId, event) {
    ...
    vueThisLocation.getSelectTasks = []
    vueThisLocation.getSelectTasks = vueThisLocation.selectedTasks
    ...
    if (taskId && !readOnly) {
      ...
      vueThisLocation.selectedTasks.push(ganttObject.getTask(taskId))
      // vueThisLocation.callParentSelectTasks()
      vueThisLocation.menuVisible = true
      let actionMenuWidth = 0
      let actionMenuHeight = 0
      Vue.nextTick(function () {
        if (vueThisLocation.$refs.actionMenu) {
          actionMenuWidth = vueThisLocation.$refs.actionMenu.offsetWidth
          actionMenuHeight = vueThisLocation.$refs.actionMenu.offsetHeight
          if (document.body.clientWidth - event.clientX < actionMenuWidth) {
            vueThisLocation.dropLeft = event.clientX - actionMenuWidth + 'px'
          } else {
            vueThisLocation.dropLeft = event.clientX + 'px'
          }
          if (document.body.clientHeight - event.clientY < actionMenuHeight) {
            vueThisLocation.dropTop = event.clientY - actionMenuHeight + 'px'
          } else {
            vueThisLocation.dropTop = event.clientY + 'px'
          }
        }
      })
    }
...
GanttObject.setOnBeforeEditStart = function (ganttObject, vueThisLocation) {
  return ganttObject.ext.inlineEditors.attachEvent('onBeforeEditStart', function (state) {
    ...
    if (vueThisLocation.createPage === 'decompose' && roles.includes('SYS_ROLE077')) {
      return false
    }
    const userMaxSecret = vueThisLocation.$store.state.user.userInfo.confidentialiteList[vueThisLocation.$store.state.user.userInfo.confidentialiteList.length - 1].id
    if (task.secretGrade > userMaxSecret) {
      vueThisLocation.$message.warning('低密人员不允许修改高密数据')
      return false
    }
    // 获取gannt操作限制策略
    ...
    if (task.monitorPoints && task.monitorPoints.indexOf('1015') !== -1 && vueThisLocation.monitorLockMap['101502'] === '1') {
      return false
    }
    return true
  })
...
GanttObject.onSaveCellEven = function (ganttObject, vueThisLocation) {
  return ganttObject.ext.inlineEditors.attachEvent('onSave', (state) => {
    ...
    if (colName === 'specialDutyDeptId') {
      ...
      ganttObject.updateTask(taskId)
      // vueThisLocation.selectedTasks = []
      // vueThisLocation.loadGanttData(vueThisLocation.planInfoId, vueThisLocation.taskId, vueThisLocation.createPage)
    }

    // 同步左下角选中任务名称
    if (colName === 'name') {
      vueThisLocation.selectTaskName = ganttObject.getTask(taskId).name
    }
  })
}
...
GanttObject.workTimeSetting = function (ganttObject, vueThisLocation) {
  ...
  // ganttObject.config.time_step = vueThisLocation.workTime
...
GanttObject.addTooltip = function (ganttObject, vueThisLocation) {
  ...
  ganttObject.attachEvent('onGanttReady', function () {
    ...
    ganttObject.templates.tooltip_text = function (start, end, task) {
      const statusData = vueThisLocation.taskStatus[task.status]
...
GanttObject.updateTaskNew = function (ganttObject, taskId, vueThisLocation) {
  ganttObject.batchUpdate(function () {
    ...
    const parTasksIds = []
    // 修改任务添加关联
    if (vueThisLocation.ganttName === 'changeGantt') {
 ...
 * @returns {boolean}
 */
export function progressRefreshCheck(vueThisLocation) {
  let result = false
  const planStatusLockMap = store.getters.planStatusLockMap
  if (planStatusLockMap[vueThisLocation.planInfoStatus] && planStatusLockMap[vueThisLocation.planInfoStatus].refreshProgress === 'true') {
    result = true
  }
  return result
}
...
/**
 * 定义列类型对应关系，此处添加后，需要编写searchFilterLocation方法对应属性过滤逻辑
 */
const columnsTypeMap = {
  statusLocation: 'select',
  managerStatusLocation: 'select',
  monitorPointsLocation: 'select',
  progressFeedbackLocation: 'select',
  planTypeLocation: 'select',
  specialDutyDeptIdLocation: 'select',
  tasksCooperateDeptLocation: 'select',
  wbsLocation: 'select',
  nameLocation: 'input',
  owner_id: 'input',
  ...
  start_dateLocation: 'date',
  end_dateLocation: 'date'
  // 'duration': 'input',
...
GanttObject.synchronizationColumns = function (vueThisLocation, ganttObject) {
  ...
  initColumns.forEach((initItem, initIndex) => {
    ...
    if (vueThisLocation.isPlan && (name === 'specialDutyDeptIdLocation' || name === 'tasksCooperateDeptLocation')) {
      type = ''
    }
    let dataIndex
    // if (type) {
    ...
    initColumns.forEach((initItem, initIndex) => {
      ...
      if (type) {
        ...
        if (type === 'select') {
          switch (name) {
            case 'statusLocation':
              datas = taskStatusArr
              break
              case 'progressFeedbackLocation':
              datas = taskProgressFeedbackArr
              break
              case 'overdueRemainingDaysLocation':
              datas = taskOverdueRemainingDaysArr
              break
            case 'weatherControlLocation':
              datas = taskWeatherControlArr
              break
            case 'managerStatusLocation':
              if (vueThisLocation.managerStatusMap && Object.keys(vueThisLocation.managerStatusMap).length > 0) {
                for (const item in vueThisLocation.managerStatusMap) {
                  const obj = {
                    id: item,
                    title: vueThisLocation.managerStatusMap[item].cmeaning
                  }
                  datas.push(obj)
                }
              }
              break
            case 'monitorPointsLocation':
              datas = ganttObject.serverList(ganttObject.config.monitor_point)
              break
            case 'planTypeLocation':
              datas = ganttObject.serverList(ganttObject.config.plan_type)
              break
            case 'specialDutyDeptIdLocation':
              ...
              break
            case 'tasksCooperateDeptLocation':
              ...
              break
            case 'secretGradeLocation':
              datas = ganttObject.serverList('secretGrades')
              break
            case 'wbsLocation':
              for (let i = 0; i < vueThisLocation.deep; i++) {
                const item = {
                  id: i + 1 + '',
                  title: i + 1 + ''
                }
          ...
          if (vueThisLocation[datePickerKey] && children && children.length) {
            // vueThisLocation[datePickerKey]: 说明组件被创建
            ...
            vueThisLocation[datePickerKey] = new Datepicker(`.${datePickerKey}`, {
          ...
          if (vueThisLocation[inputKey] && children && children.length) {
            // vueThisLocation[inputKey]: 说明组件被创建
            ...
            vueThisLocation[inputKey] = new Inputor(`.${inputKey}`, {
              value: vueThisLocation.searchForm[name] || '',
              placeholder: '请输入',
              onChangeValue(value) {
          ...
          // 列resize后数据回填
          if (vueThisLocation.searchForm && Object.keys(vueThisLocation.searchForm).length) {
            const obj = document.getElementById(name + type)
            let result
            if (obj) {
              if (vueThisLocation.searchForm[name] && vueThisLocation.searchForm[name] instanceof Array) {
                result = vueThisLocation.searchForm[name].join(',')
              } else if (vueThisLocation.searchForm[name]) {
                result = vueThisLocation.searchForm[name]
              } else {
                result = ''
              }
        ...
        if (datas && datas.length > 0) {
          ...
          if (vueThisLocation[selectorKey] && children && children.length) {
            // vueThisLocation[selectorKey]: 说明select被创建
            ...
            const obj = new Selector(`.${selectorKey}`, {
              ...
              value: vueThisLocation.searchForm[name] || vueThisLocation.searchForm__WBS || '', // 绑定的值[array or string](multiple为true--value: ['yk', 'p8'] or value: 'yk,p8'; multiple为false--value: 'yk')
              placeholder: '请选择', // 默认提示文本
              onSelect: function (value) {
                // select change事件
                if (name === 'wbsLocation') {
                  // 列->大纲: 查询和其他列查询方式不同: 大纲列单独处理,选中的值单独存放; 其余列查询按照值进行包含匹配
                  vueThisLocation.searchForm__WBS = value
                  ganttObject.eachTask(function (task) {
            ...
            vueThisLocation[selectorKey] = obj
          }
        }
      }
    })
