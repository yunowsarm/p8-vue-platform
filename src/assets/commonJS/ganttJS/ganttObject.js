...
function searchFilter(parent, searchForm, ganttObject) {
  if (JSON.stringify(searchForm) !== '{}') {
    ...
    if (monitors && monitors.length > 0 && !GanttObject.calculateArrayContain(task.monitorPoints, monitors)) {
      monitorsCheck = false
    }
    let monitorPoints = searchForm.monitorPoints // 标识--列头
    if (monitorPoints && monitorPoints.length > 0 && !GanttObject.calculateArrayContain(task.monitorPoints, monitorPoints)) {
      monitorsCheck = false
    }
...
GanttObject.synchronizationColumns = function (vueThis, ganttObject) {
  ...
  // 获取gantt列配置信息
  let ganttSetting = GanttObject.getGanttSettingGrid(vueThis.ganttName, vueThis.createPage)
