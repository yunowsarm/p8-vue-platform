 ...
 *
 */
export const generateTreeThree = (treeData, pidName) => {
  const pidNameStr = pidName != null && pidName !== '' ? pidName : 'pId'
  // // 把跟节点首先放进数组
  const tmpTree = treeData.filter((node) => !node[pidNameStr] || node[pidNameStr] === '0')

  // 递归生成节点及子节点数据
  const findChildren = (tree, parentWBS, WBS) => {
    tree.forEach((node) => {
      if (parentWBS) {
        node.WBS = parentWBS + '.' + ++WBS
      } else {
        node.WBS = ++WBS
      }
      node.children = treeData.filter((cNode) => cNode[pidNameStr] === node.id)
      if (node.children.length) {
        findChildren(node.children, node.WBS, 0)
      }
    })
  }
  findChildren(tmpTree, null, 0)
